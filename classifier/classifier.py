...
if __name__ == '__main__':
    ...
    target_tweets = df.Text
    target_tweets = [x for x in target_tweets if type(x) == str]
    tweet_predictions = get_tweets_predictions(target_tweets)

    print("Printing predicted values: ")
    with open('results.csv', mode='w') as pred_file:
        pred_file = csv.writer(pred_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)    
        pred_file.writerow(['tweet', 'prediction'])
        for i,t in enumerate(target_tweets):
            print(t)
            print(class_to_name(tweet_predictions[i]))
            pred_file.writerow([t, class_to_name(tweet_predictions[i])])
